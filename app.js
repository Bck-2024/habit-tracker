...
function getAuthToken() {
  return localStorage.getItem("good_bck_token") || "";
}

async function loginWithPassword() {
  const password = window.prompt("请输入访问口令");

  if (!password) {
    throw new Error("未输入访问口令");
  }
  ...
  localStorage.setItem("good_bck_token", data.token);
  return data.token;
}
...
async function fetchData() {
  ...
  let response = await fetch(`${window.APP_CONFIG.API_BASE_URL}/data`, {
    method: "GET",
    headers: {
  ...
  if (response.status === 401) {
    localStorage.removeItem("good_bck_token");
    token = await loginWithPassword();

    response = await fetch(`${window.APP_CONFIG.API_BASE_URL}/data`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${token}`
      }
...
async function init() {
  try {
    ...
    console.error(error);
    if (error.message === "Invalid password" || error.message === "未输入访问口令") {
      localStorage.removeItem("good_bck_token");
    }
